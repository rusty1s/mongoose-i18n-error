...
module.exports = function(app) {
	...
	describe('I18n-2', function() {
		...
		it('should print a localized error message', function(done) {
			...
			app.post('/i18n-2/1', function(req, res, next) {
			...
			request(app).post('/i18n-2/1').expect(422).end(function(err, res) {
		...
		it('should print a localized minlength error message', function(done) {
			var Model = mongoose.model('Model', helper.createStringSchema());
			app.post('/i18n-2/2', function(req, res, next) {
				new Model({
					value: 'ab'
				}).save(function(err) {
					if (err) return next(err);
			...
			request(app).post('/i18n-2/2').expect(422).end(function(err, res) {
		...
		it('should print a localized min error message', function(done) {
			...
			app.post('/i18n-2/3', function(req, res, next) {
				new Model({
					value: -1
				}).save(function(err) {
					if (err) return next(err);
			...
			request(app).post('/i18n-2/3').expect(422).end(function(err, res) {
